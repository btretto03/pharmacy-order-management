...
COLOR_WHATSAPP = "#25D366"   # Verde WhatsApp
...
class AppControleVendas:
    def __init__(self, root):
        ...
        if os.path.exists(config_file):
            try:
                with open(config_file, "r", encoding="utf-8") as f:
                    ...
                    if path_lido: self.db_name = path_lido
            except: pass
        ...
        try:
            ...
            self.criar_tabela()
            self.verificar_tabela_mensagens() 
        except Exception as e:
            messagebox.showerror("Erro Crítico", f"Falha ao conectar no Banco:\n{e}")
            sys.exit()
    ...
    def iniciar_loop_atualizacao(self):
        try: self.carregar_dados(manter_selecao=True)
        except: pass 
    ...
    def criar_cabecalho(self):
        ...
        # --- ÁREA DO FATURAMENTO (Lado Direito Extremo) ---
        ...
        # --- ÁREA DE BOTÕES (Agrupados em "Retângulo") ---
        frame_btns = tk.Frame(header_frame, bg=COLOR_PRIMARY)
        frame_btns.pack(side="right", padx=20)

        btn_desp = tk.Button(frame_btns, text="💲 DESPESAS", bg=COLOR_SECONDARY, fg=COLOR_PRIMARY, 
                             font=("Arial", 9, "bold"), bd=1, relief="raised", cursor="hand2",
                             command=self.pedir_senha_custom) 
        btn_desp.pack(fill="x", pady=(0, 3)) # Espaço em baixo

        btn_whats = tk.Button(frame_btns, text="📱 WHATSAPP", bg=COLOR_WHATSAPP, fg="white",
                              font=("Arial", 9, "bold"), bd=1, relief="raised", cursor="hand2",
                              command=self.clique_whats_main)
        btn_whats.pack(fill="x", pady=(3, 0)) # Espaço em cima
    ...
    def pedir_senha_custom(self):
        ...
        tk.Button(pop_senha, text="ENTRAR", bg=COLOR_PRIMARY, fg="white", command=verificar).pack(pady=10)
    ...
    def adicionar_despesa(self):
        try:
            ...
            aviso_zerado = False
            if not val_str:
                if not messagebox.askyesno("Confirmar", "Valor não informado, deseja prosseguir?", parent=self.top_desp): return
                valor = 0.0
                aviso_zerado = True
            else: valor = float(val_str)
            ...
            self.carregar_dados_despesas()
            self.entry_desc_desp.focus_set() 
            if not aviso_zerado: messagebox.showinfo("Sucesso", "Conta lançada!", parent=self.top_desp)
        except ValueError: messagebox.showerror("Erro", "Valor inválido", parent=self.top_desp)

    def carregar_dados_despesas(self, event=None):
        for i in self.tree_desp.get_children(): self.tree_desp.delete(i)
        ...
        if mes != 'Todos': filtros_data.append(f"strftime('%m', vencimento) = '{MESES_FILTRO[mes]}'")
        if ano != 'Todos': filtros_data.append(f"strftime('%Y', vencimento) = '{ano}'")
        if filtros_data: query += " AND (" + " AND ".join(filtros_data) + " OR vencimento = '' OR vencimento IS NULL)"
        ...
        for row in rows:
            ...
            self.tree_desp.insert("", "end", values=(row[0], row[1], row[2], val_fmt, venc_br, row[5]), tags=(tag,))
            total += row[3]
    ...
    def abrir_edicao_despesa(self, event):
        ...
        if venc_atual != "A DEFINIR" and venc_atual != "SEM DATA": edit_venc.insert(0, venc_atual)
        ...
        if status_atual == "A LANÇAR": edit_status.set("PENDENTE")
        else: edit_status.set(status_atual)
        ...
        def salvar():
            try:
                ...
                try: n_venc_db = datetime.strptime(n_venc_br, '%d/%m/%Y').strftime("%Y-%m-%d")
                except: n_venc_db = ""
    ...
    def pagar_despesa(self):
        ...
        if str(id_conta).startswith("FIX_"):
            ...
            ano = self.combo_ano_desp.get()
            if mes_idx and ano != 'Todos': venc_db = f"{ano}-{mes_idx}-10"
            else: venc_db = datetime.now().strftime("%Y-%m-%d")
            self.cursor.execute("INSERT INTO despesas (descricao, categoria, valor, vencimento, status) VALUES (?, ?, ?, ?, ?)",
                                (desc_real, 'Fixa', val_real, venc_db, 'PAGO'))
        else: self.cursor.execute("UPDATE despesas SET status='PAGO' WHERE id=?", (id_conta,))
        self.conn.commit()
        self.carregar_dados_despesas()
    ...
    def deletar_despesa(self):
        ...
        if str(id_conta).startswith("FIX_"):
            if "||" in str(id_conta): desc_real = str(id_conta).split("||")[1]
            else: desc_real = desc_conta
            if messagebox.askyesno("Excluir Recorrência", f"Deseja excluir '{desc_real}' de TODOS os meses?"):
        ...
        if cat_conta == 'Fixa':
            resposta = messagebox.askyesnocancel("Excluir Despesa Fixa", "SIM: Exclui de TODOS os meses\nNÃO: Exclui APENAS deste mês\nCANCELAR: Não faz nada")
            if resposta is None: return 
            if resposta: self.cursor.execute("DELETE FROM despesas WHERE descricao=? AND categoria='Fixa'", (desc_conta,))
            else: self.cursor.execute("DELETE FROM despesas WHERE id=?", (id_conta,))
        else:
    ...
    def criar_area_filtros(self):
        ...
        tk.Label(frame_topo, text="Pesquisar:", **lbl_style).pack(side="left", padx=(30, 5))
    ...
    def criar_rodape(self):
        ...
        # BOTÃO WHATSAPP NO RODAPÉ REMOVIDO PARA EVITAR DUPLICIDADE (Já está no header)
        # Se quiser manter em baixo também, descomente as linhas abaixo:
        # btn_whats = tk.Button(frame_footer, text="📱 WhatsApp", bg=COLOR_WHATSAPP, fg="white", 
        #                       command=self.clique_whats_main, **btn_style)
        # btn_whats.pack(side="left", padx=15)
    ...
    def criar_tabela(self):
        ...
        """)
        try: self.cursor.execute("SELECT telefone FROM pedidos LIMIT 1")
        except: 
    ...
    def verificar_tabela_mensagens(self):
        """Cria tabela de mensagens personalizadas"""
        self.cursor.execute("""
    ...
    def abrir_popup_calendario(self, entry_alvo):
        if isinstance(entry_alvo, tk.Entry): parent = entry_alvo.winfo_toplevel()
        ...
        cal = Calendar(top_cal, selectmode='day', locale='pt_BR', date_pattern='dd/mm/yyyy',
                       showweeknumbers=False, background=COLOR_PRIMARY, foreground='white',
    ...
    def atualizar_lista_clientes(self, event):
        ...
        else: self.listbox_clientes.place_forget()
    ...
    def atualizar_lista_produtos(self, event):
        ...
        else: self.listbox_produtos.place_forget()
    ...
    def clique_whats_main(self):
        """Ação do botão WhatsApp (No Header)"""
    ...
    def abrir_editor_mensagens(self):
        ...
        def salvar_modelo():
            ...
            # Verifica se é edição ou novo (pela seleção da lista)
            sel = listbox_msgs.curselection()
            if sel:
    ...
    def abrir_janela_edicao(self, event):
        ...
        tk.Label(frame_conteudo, text="Nome Cliente:", **lbl_style).pack(fill="x", pady=(5,0))
        ...
        edit_telefone = tk.Entry(fr_tel, width=18, **entry_style)
        edit_telefone.pack(side="left", padx=5)
        if telefone_db: edit_telefone.insert(0, telefone_db)
        
        # BOTÃO WHATSAPP (NOVO MENU)
        def call_menu():
            self.abrir_menu_whats(edit_telefone.get(), edit_nome.get(), 
                                  edit_produto.get(), edit_valor.get(), edit_data_entrega.get())

        btn_whats = tk.Button(fr_tel, text="📱 Opções Whats", bg=COLOR_WHATSAPP, fg="white", font=("Arial", 9, "bold"),
                              command=call_menu)
        btn_whats.pack(side="left", padx=10)
        ...
        tk.Label(frame_conteudo, text="Produto:", **lbl_style).pack(fill="x", pady=(5,0))
        edit_produto = tk.Entry(frame_conteudo, **entry_style)
        edit_produto.pack(fill="x", pady=(0,5))
        ...
        frame_row3.pack(fill="x", pady=10)
    ...
    def carregar_dados(self, event=None, manter_selecao=False):
        ...
        if nome_buscado:
            query += " AND (nome LIKE ? OR cidade LIKE ? OR produto LIKE ?)"
            term = f"%{nome_buscado}%"
        ...
        try:
            ...
            for row in rows:
                ...
                # Telefone pode ser row[9] se existir
                ...
                # ADICIONADO TELEFONE NA LISTAGEM
